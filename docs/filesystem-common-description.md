### Description of the Cheesy filesystem's common mechanisms

The Cheesy filesystem was designed to be simple and fast (see KISS). It uses trees to store directories and files descriptors called `nodes`. *Each node can describe both folder and file*.
Since `struct node` has a fixed size, Cheesy filesystem manages metadata space via bitmaps. Also, since we know that the space is supposed to be allocated for a single `node` only, Cheesy filesystem doesn't have to store every chunk size (i.e. we guarantee that each request to the bitmap wants a fixed well-known amount of memory). So the bitmap has to be able to define and switch 1 bit in its descriptors.
...
Note that the bits a directory node uses to hold pointer to the first child, 
